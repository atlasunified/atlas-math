...
MODULE_INFO = {
    "module_id": "number_theory.arithmetic_functions.divisors_count",
    "name": "Divisors Count",
    "topic": "number_theory",
    "subtopic": "arithmetic_functions.divisors_count",
    "difficulty_levels": ["level_1","level_2","level_3","level_4","level_5"],
    "enabled": True,
}

INSTRUCTIONS = [
    "Find the number of positive divisors of {problem}.",
    "Compute d(n) for {problem}.",
]
...
def _build(rng: random.Random, difficulty: str):
    ...
    for _, e in factors:
        d *= (e + 1)
    factor_text = " * ".join(f"{p}^{e}" if e > 1 else str(p) for p, e in factors)
    problem = f"n = {n}"
    answer = f"{d}"
    metadata = {"prime_factorization": factor_text, "factor_exponents": factors}
