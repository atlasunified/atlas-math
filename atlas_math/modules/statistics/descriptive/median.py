...
MODULE_INFO = {
    "module_id": "statistics.descriptive.median",
    "name": "Median",
    "topic": "statistics",
    "subtopic": "descriptive.median",
    "difficulty_levels": ["level_1","level_2","level_3","level_4","level_5"],
    "enabled": True,
}

INSTRUCTION_TEMPLATES = ["Find the median of {problem}.","Compute the median for {problem}.","Determine the middle value in {problem}."]
...
def _build_problem(rng: random.Random, difficulty: str):
    ...
    s = _sorted(data)
    med = _median(s)
    ans = str(int(med)) if abs(med - int(med)) < 1e-9 else f"{med:.1f}"
    metadata = {"sample_size": len(data), "already_sorted": False}
