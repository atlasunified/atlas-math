...
MODULE_INFO = {
    "module_id": "statistics.data_displays.dot_plots",
    "name": "Dot Plots",
    "topic": "statistics",
    "subtopic": "data_displays.dot_plots",
    "difficulty_levels": ["level_1","level_2","level_3","level_4","level_5"],
    "enabled": True,
}

INSTRUCTION_TEMPLATES = ["Summarize the dot plot counts for {problem}.","Find the dot-plot frequencies for {problem}.","Count the dots for each value in {problem}."]
...
def _build_problem(rng: random.Random, difficulty: str):
    ...
    fm = _freq_map(data)
    ans = "; ".join(f"{k}:{'.'*v}" for k, v in fm.items())
    metadata = {"distinct_values": len(fm), "sample_size": len(data)}
