...
MODULE_INFO = {
    "module_id": "trigonometry.equations.inverse_trig_equations",
    "name": "Inverse Trig Equations",
    "topic": "trigonometry",
    "subtopic": "equations.inverse_trig_equations",
...
def _build_problem(rng: random.Random, difficulty: str):
    if difficulty in ('level_1', 'level_2', 'level_3'):
        expr, ans = rng.choice(CASES)
        return f'Evaluate {expr}.', ans, {'task': 'principal_value', 'expression': expr}
    expr, ans = rng.choice(COMPOSED)
    return f'Evaluate {expr}.', ans, {'task': 'composed_inverse_trig', 'expression': expr}
