...
MODULE_INFO = {
    "module_id": "trigonometry.equations.basic_trig_equations",
    "name": "Basic Trig Equations",
    "topic": "trigonometry",
    "subtopic": "equations.basic_trig_equations",
...
SIN_CASES = [('0', '0, π'), ('1', 'π/2'), ('-1', '3π/2'), ('1/2', 'π/6, 5π/6'), ('√2/2', 'π/4, 3π/4')]
COS_CASES = [('1', '0, 2π'), ('0', 'π/2, 3π/2'), ('-1', 'π'), ('1/2', 'π/3, 5π/3'), ('-1/2', '2π/3, 4π/3')]
TAN_CASES = [('0', '0, π'), ('1', 'π/4, 5π/4'), ('-1', '3π/4, 7π/4'), ('√3', 'π/3, 4π/3'), ('√3/3', 'π/6, 7π/6')]


def _build_problem(rng: random.Random, difficulty: str):
    if difficulty in ('level_1', 'level_2'):
        val, sol = rng.choice(SIN_CASES)
        return f'Solve sin(x) = {val} for 0 ≤ x < 2π.', sol, {'task': 'solve_interval', 'function': 'sin', 'value': val}
    if difficulty == 'level_3':
        val, sol = rng.choice(COS_CASES)
        return f'Solve cos(x) = {val} for 0 ≤ x < 2π.', sol, {'task': 'solve_interval', 'function': 'cos', 'value': val}
    if difficulty == 'level_4':
        val, sol = rng.choice(TAN_CASES)
        return f'Solve tan(x) = {val} for 0 ≤ x < 2π.', sol, {'task': 'solve_interval', 'function': 'tan', 'value': val}
    func = rng.choice(['sin', 'cos'])
    cases = SIN_CASES if func == 'sin' else COS_CASES
    val, sol = rng.choice(cases)
    return f'Solve {func}(θ) = {val} for 0 ≤ θ < 2π.', sol, {'task': 'solve_interval', 'function': func, 'value': val}
