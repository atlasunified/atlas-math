...
MODULE_INFO = {
    "module_id": "trigonometry.equations.trig_equations_general_solutions",
    "name": "Trig Equations General Solutions",
    "topic": "trigonometry",
    "subtopic": "equations.trig_equations_general_solutions",
...
SOLNS = [
    ...
    ('sin(x) = -1', 'x = 3π/2 + 2nπ, n ∈ ℤ'),
    ('tan(x) = 1', 'x = π/4 + nπ, n ∈ ℤ'),
    ('tan(x) = -1', 'x = 3π/4 + nπ, n ∈ ℤ'),
]


def _build_problem(rng: random.Random, difficulty: str):
    eq, sol = rng.choice(SOLNS)
    if difficulty in ('level_4', 'level_5') and eq == 'sin(x) = 0':
        eq, sol = 'sin(2x) = 0', 'x = nπ/2, n ∈ ℤ'
    elif difficulty == 'level_5' and eq == 'cos(x) = -1':
        eq, sol = 'cos(3x) = -1', 'x = π/3 + 2nπ/3, n ∈ ℤ'
    return f'Find the general solution of {eq}.', sol, {'task': 'general_solution', 'equation': eq}
