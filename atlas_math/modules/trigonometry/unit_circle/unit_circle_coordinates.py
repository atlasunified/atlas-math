...
MODULE_INFO = {
    "module_id": "trigonometry.unit_circle.unit_circle_coordinates",
    "name": "Unit Circle Coordinates",
    "topic": "trigonometry",
    "subtopic": "unit_circle.unit_circle_coordinates",
    "difficulty_levels": ["level_1", "level_2", "level_3", "level_4", "level_5"],
    "enabled": True,
}

INSTRUCTION_TEMPLATES = [
    "Give the terminal-point coordinates for {problem}.",
    "Find the point on the unit circle described by {problem}.",
    "Determine the unit-circle coordinates for {problem}.",
]
...
def _choose_angle(rng, difficulty):
    if difficulty == 'level_1':
        return rng.choice([0, 30, 45, 60, 90, 180, 270])
    if difficulty == 'level_2':
        return rng.choice(ANGLES)
    if difficulty == 'level_3':
        return rng.choice([30,45,60,120,135,150,210,225,240,300,315,330])
    if difficulty == 'level_4':
        return rng.choice(ANGLES)
    k = rng.randint(-3, 5)
    return rng.choice(ANGLES) + 360 * k


def _build_problem(rng, difficulty):
    angle = _choose_angle(rng, difficulty)
    display_angle = norm_angle(angle) if difficulty != 'level_5' else angle
    use_radians = difficulty in {'level_3', 'level_4'} or (difficulty == 'level_5' and rng.random() < 0.6)
    problem = f"θ = {angle_to_radians(display_angle) if use_radians else str(display_angle)+'°'}"
    base = angle % 360
    x, y = SPECIAL[base]
    return problem, f"({x}, {y})", {'angle_degrees': angle, 'angle_display_degrees': display_angle, 'angle_radians': angle_to_radians(display_angle), 'representation': 'radians' if use_radians else 'degrees'}
