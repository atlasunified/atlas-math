...
MODULE_INFO = {
    "module_id": "trigonometry.unit_circle.terminal_point_coordinates",
    "name": "Terminal Point Coordinates",
    "topic": "trigonometry",
    "subtopic": "unit_circle.terminal_point_coordinates",
    "difficulty_levels": ["level_1", "level_2", "level_3", "level_4", "level_5"],
    "enabled": True,
}

INSTRUCTION_TEMPLATES = [
    "Use the terminal-point definition to answer {problem}.",
    "Find the requested coordinate information in {problem}.",
    "Determine the terminal point for {problem}.",
]
...
def _build_problem(rng, difficulty):
    ...
    if difficulty == 'level_1':
        problem = f"On the unit circle, if θ = {angle}°, find the x-coordinate of the terminal point"
        answer = x
        meta = {'coordinate': 'x', 'angle_degrees': angle}
    elif difficulty == 'level_2':
        coord = rng.choice(['x', 'y'])
        problem = f"On the unit circle, if θ = {angle_to_radians(angle)}, find the {coord}-coordinate of the terminal point"
        answer = x if coord == 'x' else y
        meta = {'coordinate': coord, 'angle_degrees': angle}
    elif difficulty == 'level_3':
        problem = f"The terminal point for θ has x = {x} and y = {y}. Give one angle between 0° and 360°"
        answer = f"{angle}°"
        meta = {'task': 'recover_angle_from_coordinates', 'representation': 'degrees'}
    elif difficulty == 'level_4':
        coord = rng.choice(['x', 'y'])
        problem = f"The terminal point of θ on the unit circle is ({x}, {y}). Find {coord}"
        answer = x if coord == 'x' else y
        meta = {'task': 'read_coordinate_from_point', 'coordinate': coord}
    else:
        problem = f"The terminal point is ({x}, {y}). Give one angle in radians between 0 and 2π"
        answer = angle_to_radians(angle)
        meta = {'task': 'recover_angle_from_coordinates', 'representation': 'radians'}
