...
MODULE_INFO = {
    "module_id": "trigonometry.unit_circle.evaluate_trig_from_unit_circle",
    "name": "Evaluate Trig from Unit Circle",
    "topic": "trigonometry",
    "subtopic": "unit_circle.evaluate_trig_from_unit_circle",
    "difficulty_levels": ["level_1", "level_2", "level_3", "level_4", "level_5"],
    "enabled": True,
}

INSTRUCTION_TEMPLATES = [
    "Evaluate the trig function in {problem}.",
    "Use the unit circle to answer {problem}.",
    "Find the exact value requested by {problem}.",
]
...
def _build_problem(rng, difficulty):
    fn = rng.choice(_func_set(difficulty))
    angle = rng.choice(ANGLES)
    display = angle_to_radians(angle) if difficulty in {'level_3','level_4','level_5'} and rng.random() < 0.6 else f'{angle}°'
    problem = f"{fn}({display})"
    answer = FUNC_VALUES[fn][angle]
    meta = {'function': fn, 'angle_degrees': angle, 'quadrant': quadrant(angle), 'undefined': answer == 'undefined'}
