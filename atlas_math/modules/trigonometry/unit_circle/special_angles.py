...
MODULE_INFO = {
    "module_id": "trigonometry.unit_circle.special_angles",
    "name": "Special Angles",
    "topic": "trigonometry",
    "subtopic": "unit_circle.special_angles",
    "difficulty_levels": ["level_1", "level_2", "level_3", "level_4", "level_5"],
    "enabled": True,
}

INSTRUCTION_TEMPLATES = [
    "Identify the special-angle measure described by {problem}.",
    "Convert the special angle in {problem}.",
    "Give the matching angle measure for {problem}.",
]

ANGLE_SET = [30, 45, 60, 120, 135, 150, 210, 225, 240, 300, 315, 330]
...
def _build_problem(rng, difficulty):
    angle = rng.choice(ANGLE_SET)
    if difficulty == 'level_1':
        problem = f"{angle}° in radians"
        answer = angle_to_radians(angle)
        meta = {'conversion': 'degrees_to_radians', 'special_family': REF_ANGLES[angle]}
    elif difficulty == 'level_2':
        problem = f"{angle_to_radians(angle)} in degrees"
        answer = f"{angle}°"
        meta = {'conversion': 'radians_to_degrees', 'special_family': REF_ANGLES[angle]}
    elif difficulty == 'level_3':
        ref = REF_ANGLES[angle]
        problem = f"the reference angle of {angle}°"
        answer = f"{ref}°"
        meta = {'task': 'reference_angle', 'quadrant': quadrant(angle)}
    elif difficulty == 'level_4':
        ref = REF_ANGLES[angle]
        problem = f"an angle in Quadrant {quadrant(angle)} with reference angle {ref}°"
        answer = f"{angle}°"
        meta = {'task': 'build_from_quadrant_and_reference', 'reference_angle': ref}
    else:
        ref = REF_ANGLES[angle]
        use_radians = rng.random() < 0.5
        if use_radians:
            problem = f"an angle in Quadrant {quadrant(angle)} with reference angle {angle_to_radians(ref)}"
            answer = angle_to_radians(angle)
            meta = {'task': 'build_from_quadrant_and_reference', 'reference_angle_radians': angle_to_radians(ref), 'answer_unit': 'radians'}
        else:
            problem = f"{angle_to_radians(angle)} has reference angle ?"
            answer = angle_to_radians(ref)
            meta = {'task': 'reference_angle', 'quadrant': quadrant(angle), 'answer_unit': 'radians'}
