...
MODULE_INFO = {
    "module_id": "trigonometry.trig_functions.even_odd_identities",
    "name": "Even-Odd Identities",
    "topic": "trigonometry",
    "subtopic": "trig_functions.even_odd_identities",
    "difficulty_levels": ["level_1", "level_2", "level_3", "level_4", "level_5"],
    "enabled": True,
}

INSTRUCTION_TEMPLATES = [
    "Use even-odd identities to answer {problem}.",
    "Simplify the trig expression in {problem}.",
    "Apply parity properties to {problem}.",
]
...
def _build_problem(rng, difficulty):
    ...
    if difficulty <= 'level_3':
        ...
        if fn in EVEN:
            ...
            answer = f"-{fn}({angle}°)"
        meta = {'function': fn, 'angle_degrees': angle, 'task': 'rewrite'}
    else:
        value = FUNC_VALUES[fn][angle]
        problem = f"Evaluate {fn}(-{angle_to_radians(angle) if difficulty == 'level_5' else str(angle)+'°'})"
        if fn in EVEN:
            answer = value
        else:
            answer = 'undefined' if value == 'undefined' else (value[1:] if value.startswith('-') else f'-{value}') if value != '0' else '0'
        meta = {'function': fn, 'angle_degrees': angle, 'task': 'evaluate'}
