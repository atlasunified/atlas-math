...
MODULE_INFO = {
    "module_id": "trigonometry.trig_functions.function_domain_range",
    "name": "Function Domain and Range",
    "topic": "trigonometry",
    "subtopic": "trig_functions.function_domain_range",
    "difficulty_levels": ["level_1", "level_2", "level_3", "level_4", "level_5"],
    "enabled": True,
}

INSTRUCTION_TEMPLATES = [
    "Give the domain or range requested in {problem}.",
    "State the correct interval information for {problem}.",
    "Identify the domain/range fact used in {problem}.",
]

INFO = {
    'sin': {'domain': '(-∞, ∞)', 'range': '[-1, 1]'},
    'cos': {'domain': '(-∞, ∞)', 'range': '[-1, 1]'},
    'tan': {'domain': 'all real x except π/2 + kπ, k ∈ ℤ', 'range': '(-∞, ∞)'},
    'csc': {'domain': 'all real x except kπ, k ∈ ℤ', 'range': '(-∞, -1] ∪ [1, ∞)'},
    'sec': {'domain': 'all real x except π/2 + kπ, k ∈ ℤ', 'range': '(-∞, -1] ∪ [1, ∞)'},
    'cot': {'domain': 'all real x except kπ, k ∈ ℤ', 'range': '(-∞, ∞)'},
}
...
def _build_problem(rng, difficulty):
    fn = rng.choice(['sin','cos'] if difficulty == 'level_1' else ['sin','cos','tan'] if difficulty == 'level_2' else ['sin','cos','tan','csc','sec','cot'])
    ask = 'range' if difficulty in {'level_1','level_3','level_5'} else 'domain'
    problem = f"the {ask} of y = {fn}(x)"
    answer = INFO[fn][ask]
    meta = {'function': fn, 'asks_for': ask}
