...
MODULE_INFO = {
    "module_id": "trigonometry.graphs.graph_transformations",
    "name": "Graph Transformations",
    "topic": "trigonometry",
    "subtopic": "graphs.graph_transformations",
...
def _equation(a, b, h_num, h_den, k, func):
    shift = 'x'
    if h_num:
        shift = f'x - {frac_str(h_num, h_den)}'
    bpart = shift if b == 1 else f'{b}({shift})'
    core = f'{func}({bpart})'
    if a != 1:
        core = f'{a}{core}'
    if k > 0:
...
def _build_problem(rng: random.Random, difficulty: str):
    ...
    if difficulty == 'level_1':
        eq = _equation(2, 1, 0, 1, 0, func)
        ans = 'vertical stretch by factor 2 (amplitude 2)'
        return f'Describe the transformation from y = {func}(x) to y = {eq}.', ans, {'task': 'describe_transform', 'equation': eq}
    if difficulty == 'level_2':
        eq = _equation(1, 1, 0, 1, -3, func)
        ans = 'shift down 3 units'
        return f'Describe the transformation from y = {func}(x) to y = {eq}.', ans, {'task': 'describe_transform', 'equation': eq}
    if difficulty == 'level_3':
        eq = _equation(1, 1, 1, 2, 0, func)
        ans = 'shift right 1/2 unit'
        return f'Describe the transformation from y = {func}(x) to y = {eq}.', ans, {'task': 'describe_transform', 'equation': eq}
    if difficulty == 'level_4':
        eq = _equation(1, 3, 0, 1, 0, func)
        ans = 'horizontal compression by factor 1/3; period becomes 2π/3'
        return f'Describe the transformation from y = {func}(x) to y = {eq}.', ans, {'task': 'describe_transform', 'equation': eq}
    eq = _equation(-2, 2, 3, 4, 1, func)
    ans = 'reflection across x-axis, vertical stretch by 2, horizontal compression by factor 1/2, shift right 3/4, shift up 1'
    return f'Describe all transformations from y = {func}(x) to y = {eq}.', ans, {'task': 'describe_transform', 'equation': eq}
