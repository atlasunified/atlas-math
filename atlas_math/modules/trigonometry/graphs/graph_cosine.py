...
MODULE_INFO = {
    "module_id": "trigonometry.graphs.graph_cosine",
    "name": "Graph Cosine",
    "topic": "trigonometry",
    "subtopic": "graphs.graph_cosine",
...
def _build_problem(rng: random.Random, difficulty: str):
    if difficulty == 'level_1':
        ans = '(0,1), (π/2,0), (π,-1), (3π/2,0), (2π,1)'
        return 'List the five key points for y = cos(x) on [0, 2π].', ans, {'task': 'key_points', 'function': 'cos'}
    if difficulty == 'level_2':
        x = rng.choice(['0', 'π/2', 'π', '3π/2', '2π'])
        val = {'0':'1','π/2':'0','π':'-1','3π/2':'0','2π':'1'}[x]
        return f'For y = cos(x), find the y-value when x = {x}.', val, {'task': 'evaluate_graph', 'function': 'cos', 'x': x}
    if difficulty == 'level_3':
        desc = 'amplitude 1, period 2π, midline y = 0, range [-1, 1]'
        return 'State the amplitude, period, midline, and range of y = cos(x).', desc, {'task': 'features', 'function': 'cos'}
    if difficulty == 'level_4':
        x = rng.choice(['π/3', '2π/3', '4π/3', '5π/3'])
        val = {'π/3':'1/2','2π/3':'-1/2','4π/3':'-1/2','5π/3':'1/2'}[x]
        return f'On the graph of y = cos(x), what is the y-value at x = {x}?', val, {'task': 'special_angle_eval', 'function': 'cos', 'x': x}
    x = rng.choice(['π/4', '3π/4', '5π/4', '7π/4'])
    val = {'π/4':'√2/2','3π/4':'-√2/2','5π/4':'-√2/2','7π/4':'√2/2'}[x]
    return f'Using the cosine graph, determine cos({x}).', val, {'task': 'graph_read', 'function': 'cos', 'x': x}
