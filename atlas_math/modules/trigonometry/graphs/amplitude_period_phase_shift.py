...
MODULE_INFO = {
    "module_id": "trigonometry.graphs.amplitude_period_phase_shift",
    "name": "Amplitude Period Phase Shift",
    "topic": "trigonometry",
    "subtopic": "graphs.amplitude_period_phase_shift",
...
def _build_problem(rng: random.Random, difficulty: str):
    ...
    if difficulty == 'level_1':
        return f'For y = 3{func}(x), find the amplitude.', '3', {'task': 'amplitude', 'function': func}
    if difficulty == 'level_2':
        b = rng.choice([2, 3, 4])
        return f'For y = {func}({b}x), find the period.', f'2π/{b}', {'task': 'period', 'function': func, 'b': b}
    if difficulty == 'level_3':
        return f'For y = 2{func}(x - π/3), find the phase shift.', 'right π/3', {'task': 'phase_shift', 'function': func}
    if difficulty == 'level_4':
        return f'For y = -4{func}(2(x + π/6)) + 1, state the amplitude, period, phase shift, and vertical shift.', 'amplitude 4, period π, phase shift left π/6, vertical shift up 1', {'task': 'all_features', 'function': func}
    b = rng.choice([Fraction(1,2), Fraction(3,2)])
    btxt = 'x/2' if b == Fraction(1,2) else '3x/2'
    period = '4π' if b == Fraction(1,2) else '4π/3'
    return f'For y = 5{func}({btxt} - π/4), find the amplitude and period.', f'amplitude 5, period {period}', {'task': 'amplitude_period', 'function': func}
